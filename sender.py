...
def send_message(sdr: adi.Pluto, message: bytes, oversampling: int, byte_per_control: int):
    ...
    for i in range(0, len(message), byte_per_control):
        ...
        msg = np.repeat([0+1j, 0-1j, *qpsk], oversampling)
...
sdr.tx_hardwaregain_chan0 = -65 # Increase for longer distance communication, valid range is between -90 to 0 dB
...
while True:
    ...
    send_message(sdr, "Hello world lorem ipsum dolor!".encode(), oversampling, byte_per_control)
